...
class ContentStorage {
  ...
  _resolveComposite(composite) {
    return Promise.each(values(composite.data.embeds), this._resolvePrimitive)
      .then(() => composite);
  }
  ...
  resolveContainer(container) {
    return Promise.each(container.elements, this._resolveElement)
      .then(() => container);
  }
  ...
  importRepo(programId, repoId) {
    ...
    return this.storage.copyDir(src, dest)
      .then(() => this._getRepository(repoId));
  }
}
